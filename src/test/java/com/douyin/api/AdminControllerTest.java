...
class AdminControllerTest {
    ...
    void statsContainsUserCount() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/admin/stats").requestAttr("userId", 1L).requestAttr("role", "ADMIN"))
                .andExpect(status().isOk())
    ...
    void requestLogsReturnsSuccessTrue() throws Exception {
        when(requestLogRepository.findAllByOrderByTimestampDescIdDesc(any(Pageable.class)))
                .thenReturn(List.of());

        mockMvc.perform(get("/api/v1/admin/request-logs").requestAttr("userId", 1L).requestAttr("role", "ADMIN"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true));
    }
    ...
    @Test
    void requestLogsReturnsCursorPaginationFields() throws Exception {
        when(requestLogRepository.findAllByOrderByTimestampDescIdDesc(any(Pageable.class)))
                .thenReturn(List.of());
